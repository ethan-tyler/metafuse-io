...
fn bench_emit_dataset_minimal(c: &mut Criterion) {
    ...
    c.bench_function("emit_dataset_minimal", |b| {
        ...
        // Initialize catalog schema once
        rt.block_on(async {
        ...
        b.to_async(&rt).iter(|| async {
            ...
            counter.set(count + 1);
            let dataset_name = format!("bench_dataset_{}", count);
...
fn bench_emit_dataset_with_lineage(c: &mut Criterion) {
    ...
    c.bench_function("emit_dataset_with_lineage", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            let count = counter.get();
            counter.set(count + 1);
            let dataset_name = format!("child_dataset_{}", count);
...
fn bench_fts_search_simple(c: &mut Criterion) {
    ...
    // Initialize and populate data once
    rt.block_on(async {
        let conn = backend.get_connection().await.unwrap();
        init_sqlite_schema(&conn).unwrap();
        drop(conn);
