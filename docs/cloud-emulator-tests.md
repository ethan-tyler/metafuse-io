...
# Run stress tests (optional, ≤30s runtime)
RUN_STRESS_TESTS=1 cargo test --test stress_tests

# Configure stress tests (optional)
...
- Stress tests validate concurrent access patterns and resource management under load
- Uses Docker containers (fake-gcs-server, MinIO) that start/stop automatically

**CI Behavior:**

- Tests skipped by default (set `RUN_CLOUD_TESTS=1` or `RUN_STRESS_TESTS=1`)
